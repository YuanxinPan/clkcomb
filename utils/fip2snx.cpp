...
{
    if (argc != 4) {
        fprintf(stderr, "usage: cvt bias year doy\n");
        return 1;
    }
    ...
    std::vector<double> wl_bias, nl_bias;
    while (fgets(buf, sizeof(buf), fp))
    {
        if (buf[0] == '*') {
            ...
            val = atof(buf+56);
            nl_bias.push_back(val);

            fprintf(stdout, "%3s %8.3f %8.3f\n", prn.c_str(), wl_bias.back(), nl_bias.back());
        }
    }

    init_sats(sats);
    write_bias("bias", year, doy, sats, wl_bias, nl_bias);
